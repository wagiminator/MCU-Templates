// ===================================================================================
// Basic System Functions for CH32V103                                        * v1.0 *
// ===================================================================================
//
...
//
// DLY_ticks(n)           // delay n * 8 system clock cycles
...
// Delay Functions
// ===================================================================================
#define STK_init()        STK->CTLR = STK_CTLR_STE      // init SYSTICK @ F_CPU / 8
#define DLY_US_TIME       (F_CPU / 8000000)             // system ticks per us
#define DLY_MS_TIME       (F_CPU / 8000)                // system ticks per ms
...
// Set VTF Interrupt
static inline void NVIC_SetFastIRQ(uint32_t addr, IRQn_Type IRQn, uint8_t num) {
  if(num > 3)  return ;
  NVIC->FIBADDRR = addr;
...
static inline void NVIC_HaltPushCfg(FunctionalState NewState) {
  if(NewState != DISABLE) NVIC->CFGR = NVIC_KEY1;
  else                    NVIC->CFGR = NVIC_KEY1|(1<<0);
}

// Enable Interrupt Nesting
static inline void NVIC_INTNestCfg(FunctionalState NewState) {
  if(NewState != DISABLE) NVIC->CFGR = NVIC_KEY1;
  else                    NVIC->CFGR = NVIC_KEY1|(1<<1);
}
...
// Return the Machine Scratch Register
