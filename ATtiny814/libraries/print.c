...
void _itoa(void (*putchar) (char c), int16_t, int8_t, int8_t);
...
static void _vfprintf(void (*putchar) (char c), const char* str,  va_list arp) {
  ...
  while((d = *str++) != 0) {
    ...
    if(r == 0) continue;
    if(r > 0) _itoa(putchar, (uint16_t)va_arg(arp, int16_t), r, w);
    else _itoa(putchar, (int16_t)va_arg(arp, int16_t), r, w);
  }
}

void _itoa(void (*putchar) (char c), int16_t val, int8_t rad, int8_t len) {
  char c, sgn = 0, pad = ' ';
  char s[10];
  uint8_t i = 0;
