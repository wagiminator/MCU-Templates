...
void SYS_init(void) {
  ...
  // Enable GPIO
  #if SYS_GPIO_EN > 0
    RCC->PB2PCENR |= RCC_IOPAEN | RCC_IOPBEN | RCC_IOPCEN | RCC_IOPDEN;
  #endif
}
...
void CLK_init_HSE(void) {
  RCC->PB2PCENR |= RCC_AFIOEN;                                  // enable auxiliary clock module
  AFIO->PCFR1   |= AFIO_PCFR1_PA1PA2_REMAP;                     // pins PA1-PA2 for external crystal
...
void MCO_init(void) {
  RCC->PB2PCENR |= RCC_AFIOEN | RCC_IOPCEN;
  GPIOC->CFGLR   = (GPIOC->CFGLR & ~((uint32_t)0b1111<<(4<<2))) | ((uint32_t)0b1001<<(4<<2));
}
...
void AWU_init(void) {
  ...
  RCC->PB1PCENR |= RCC_PWREN;           // enable power module
  PWR->AWUCSR    = PWR_AWUEN;           // enable automatic wake-up timer
}
...
// ===================================================================================
// C++ support based on CNLohr ch32v003fun: https://github.com/cnlohr/ch32v003fun
// ===================================================================================
#ifdef __cplusplus
...
// ===================================================================================
// C version of startup_ch32v00X.S file from WCH
// ===================================================================================
...
DUMMY_HANDLER void OPCM_IRQHandler(void);
